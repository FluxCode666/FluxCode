...
Embedding 账号允许直接使用公网或私网 HTTP/HTTPS 上游，无需配置域名或 CIDR 白名单。回环、链路本地、元数据、shared、documentation 与保留地址仍会被阻止，Embedding 账号也不能使用代理。完整且有硬上界的资源配置见 [`deploy/config.example.yaml`](deploy/config.example.yaml)。
