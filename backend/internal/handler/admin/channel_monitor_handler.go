...
func channelMonitorToResponse(m *service.ChannelMonitor) *channelMonitorResponse {
	...
	if m.LastCheckedAt != nil {
		checkedAt := m.LastCheckedAt.UTC().Format(time.RFC3339)
...
func (h *ChannelMonitorHandler) Run(c *gin.Context) {
	...
	for _, result := range results {
		out = append(out, checkResultToResponse(result))
	}
...
func (h *ChannelMonitorHandler) History(c *gin.Context) {
	...
	entries, err := h.monitorService.ListHistory(c.Request.Context(), id, strings.TrimSpace(c.Query("model")), parseHistoryLimit(c.Query("limit")))
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	...
	for _, entry := range entries {
		out = append(out, historyEntryToResponse(entry))
	}
