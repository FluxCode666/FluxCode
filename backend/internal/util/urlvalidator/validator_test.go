...
func TestValidateIPRejectsIANAIPv4SpecialPurposeAddresses(t *testing.T) {
	for _, raw := range []string{
		...
		t.Run(raw, func(t *testing.T) {
			if err := ValidateIP(net.ParseIP(raw)); err == nil {
				t.Fatalf("expected IANA special-purpose address %s to be rejected", raw)
			}
		})
	}
}

func TestValidateIPRejectsIANAIPv6SpecialPurposeAddresses(t *testing.T) {
	for _, raw := range []string{
		"::",
		"::1",
		"::ffff:127.0.0.1",
...
func TestValidateIPAcceptsPublicUnicastAddresses(t *testing.T) {
	for _, raw := range []string{"8.8.8.8", "::ffff:8.8.8.8", "2606:4700:4700::1111"} {
		t.Run(raw, func(t *testing.T) {
			if err := ValidateIP(net.ParseIP(raw)); err != nil {
				t.Fatalf("expected public unicast address %s to pass: %v", raw, err)
			}
		})
	}
}
