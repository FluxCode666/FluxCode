...
//
// 当前路由策略（仅基于账号手动开关，详见 openai_compat.ShouldUseResponsesAPI）：
//   - APIKey 账号 + 手动开启 Chat Completions → 走 forwardAsRawChatCompletions
//     直转上游 /v1/chat/completions，不做协议转换
//   - 其他所有情况 → 走原有 CC→Responses 转换路径
//
// 能力探测标记仅供展示，不得隐式改变协议路由。
func (s *OpenAIGatewayService) ForwardAsChatCompletions(
	...
	// 入口分流：仅 APIKey 账号显式打开 Chat Completions 开关时走 CC 直转。
