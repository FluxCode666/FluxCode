...
// ReadOpenAIWSClientMessage keeps the reader alive until a timeout or parent
// cancellation can close the socket and join it. coder/websocket permits only
// one reader, so merely cancelling a Read context can otherwise leave that
// reader blocked and retain an ingress connection indefinitely.
