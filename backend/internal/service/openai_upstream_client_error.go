...
var (
	openAIUpstreamClientErrorTypeCodePattern = regexp.MustCompile(`^[A-Za-z0-9_.-]{1,128}$`)
	openAIUpstreamClientErrorParamPattern    = regexp.MustCompile(`^[A-Za-z0-9_.\[\]-]{1,512}$`)
)
...
func writeOpenAIUpstreamClientError(c *gin.Context, statusCode int, body []byte, upstreamMsg string) {
	...
	if errType := strings.TrimSpace(gjson.GetBytes(body, "error.type").String()); openAIUpstreamClientErrorTypeCodePattern.MatchString(errType) {
		errorPayload["type"] = errType
	}
	if code := strings.TrimSpace(extractUpstreamErrorCode(body)); openAIUpstreamClientErrorTypeCodePattern.MatchString(code) {
		errorPayload["code"] = code
	}
	if param := strings.TrimSpace(gjson.GetBytes(body, "error.param").String()); openAIUpstreamClientErrorParamPattern.MatchString(param) {
