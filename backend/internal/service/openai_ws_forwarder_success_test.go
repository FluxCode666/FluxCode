...
func TestOpenAIGatewayService_Forward_WSv2_CodexImageBridgeInjectsWhenEnabled(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	captureConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_ingress_image_bridge","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	...
	account := &Account{
		ID:          611,
		Name:        "openai-ingress-image-bridge",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	wsServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		ginCtx.Request = req
		ginCtx.Set("api_key", &APIKey{Group: &Group{AllowImageGeneration: true}})
	...
	err = clientConn.Write(writeCtx, coderws.MessageText, []byte(`{"type":"response.create","model":"gpt-5.1","stream":false,"input":"draw a cat"}`))
	cancelWrite()
	require.NoError(t, err)
...
func TestOpenAIGatewayService_Forward_WSv2_DeniesImageIntentWhenGroupDisabled(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	account := &Account{
		ID:          612,
		Name:        "openai-ingress-image-deny",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeCtx, cancelWrite := context.WithTimeout(context.Background(), 3*time.Second)
	err = clientConn.Write(writeCtx, coderws.MessageText, []byte(`{"type":"response.create","model":"gpt-5.5","stream":false,"input":"draw","tools":[{"type":"image_generation"}]}`))
	cancelWrite()
	require.NoError(t, err)
	...
	case serverErr := <-serverErrCh:
		...
		require.Contains(t, closeErr.Reason(), ImageGenerationPermissionMessage())
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket 结束超时")
	}

	require.Zero(t, captureDialer.DialCount(), "group 禁用时不应建立上游 websocket")
}
