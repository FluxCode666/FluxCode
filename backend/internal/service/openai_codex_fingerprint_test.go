...
func TestOpenAIOAuthClientMetadataPreservedWhenFingerprintIsOff(t *testing.T) {
	...
	require.True(t, ok)
	require.Equal(t, "configured-device", metadata["x-codex-installation-id"])
...
func TestBuildUpstreamRequest_CodexFingerprintPreservesConversationIsolation(t *testing.T) {
	...
	c.Request.Header.Set("x-codex-turn-metadata", `{"installation_id":"client-install","session_id":"client-session","thread_id":"client-thread","turn_id":"client-turn","window_id":"client-thread:0"}`)
	...
	// FluxCode's client API-key isolation remains on conversation_id, while the
	// upstream fingerprint algorithm intentionally owns the session identifiers.
	assert.Equal(t, isolateOpenAISessionID(77, "prompt-cache-key"), req.Header.Get("conversation_id"))
	assert.Equal(t, ids.sessionID, req.Header.Get("session_id"))
	assert.Equal(t, ids.sessionID, req.Header.Get("session-id"))
