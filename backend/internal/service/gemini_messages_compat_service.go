...
// to filter out accounts whose assigned proxy has been disabled.
func (s *GeminiMessagesCompatService) SetDisabledProxyScheduleModeProvider(p DisabledProxyScheduleModeProvider) {
	if s == nil {
		return
	}
...
// disabled when the operator has chosen "exclude_account" via pool-monitor settings.
func (s *GeminiMessagesCompatService) applyDisabledProxyScheduleMode(ctx context.Context, accounts []Account) []Account {
...
// from scheduling because its assigned proxy is disabled.
func (s *GeminiMessagesCompatService) isAccountBlockedByDisabledProxy(ctx context.Context, account *Account) bool {
...
func (s *GeminiMessagesCompatService) tryStickySessionHit(
	...
	// 检查代理是否已禁用
	// Check if account's proxy is disabled
	...
	for attempt := 1; attempt <= geminiMaxRetries; attempt++ {
		...
		if err != nil {
			...
			if attempt < geminiMaxRetries {
				logger.LegacyPrintfContext(ctx, "service.gemini_messages_compat", "Gemini account %d: upstream request failed, retry %d/%d: %v", account.ID, attempt, geminiMaxRetries, err)
		...
		if resp.StatusCode == http.StatusBadRequest && signatureRetryStage < 2 {
			...
			if isGeminiSignatureRelatedError(respBody) {
				...
				if txErr == nil {
					logger.LegacyPrintfContext(ctx, "service.gemini_messages_compat", "Gemini account %d: detected signature-related 400, retrying with downgraded Claude blocks (%s)", account.ID, stageName)
		...
		if resp.StatusCode >= 400 && s.shouldRetryGeminiUpstreamError(account, resp.StatusCode) {
			...
			if attempt < geminiMaxRetries {
				...
				logger.LegacyPrintfContext(ctx, "service.gemini_messages_compat", "Gemini account %d: upstream status %d, retry %d/%d", account.ID, resp.StatusCode, attempt, geminiMaxRetries)
				sleepGeminiBackoff(attempt)
				continue
			}
	...
	if resp.StatusCode >= 400 {
		...
		if resp.StatusCode == http.StatusBadRequest {
			...
			if isGoogleProjectConfigError(msg400) {
				...
				logger.LegacyPrintfContext(ctx, "service.gemini_messages_compat", "[Gemini] status=400 google_config_error failover=true upstream_message=%q account=%d", upstreamMsg, account.ID)
				appendOpsUpstreamError(c, OpsUpstreamErrorEvent{
					Platform:           account.Platform,
					AccountID:          account.ID,
					AccountName:        account.Name,
...
func (s *GeminiMessagesCompatService) ForwardNative(ctx context.Context, c *gin.Context, account *Account, originalModel string, action string, stream bool, body []byte) (*ForwardResult, error) {
	...
	if resp.StatusCode >= 400 {
		...
		if s.cfg != nil && s.cfg.Gateway.LogUpstreamErrorBody {
			...
			upstreamDetail = truncateString(string(respBody), maxBytes)
			logger.LegacyPrintfContext(ctx, "service.gemini_messages_compat", "[Gemini] native upstream error %d: %s", resp.StatusCode, truncateForLog(respBody, s.cfg.Gateway.LogUpstreamErrorBodyMaxBytes))
		}
...
func (s *GeminiMessagesCompatService) writeGeminiMappedError(c *gin.Context, account *Account, upstreamStatus int, upstreamRequestID string, body []byte) error {
	...
	if s.cfg != nil && s.cfg.Gateway.LogUpstreamErrorBody {
		...
		logger.LegacyPrintfContext(logCtx, "service.gemini_messages_compat", "[Gemini] upstream error %d: %s", upstreamStatus, truncateForLog(body, s.cfg.Gateway.LogUpstreamErrorBodyMaxBytes))
	}
