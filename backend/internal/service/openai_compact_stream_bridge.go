...
// originally a streaming Responses request before normalization to unary.
...
// writeOpenAICompactSSEBridge converts the unary compact JSON response back to
// the minimal Responses SSE protocol expected by a body-signal stream client.
...
func writeOpenAICompactSSEFailure(c *gin.Context, statusCode int, errorBody []byte) {
	...
	writeOpenAICompactSSEFailureMessage(c, "upstream_error", message)
}

func writeOpenAICompactSSEFailureMessage(c *gin.Context, errType, message string) {
	if c == nil {
		return
	}
