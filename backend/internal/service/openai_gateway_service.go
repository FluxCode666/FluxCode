...
const (
	...
	// 陈旧快照不能继续驱动调度暂停；放行一次请求后由新响应头自愈。
	openAICodexAutoPauseStaleAfter = 2 * time.Hour
	// 对齐上游首输出 staging 上限，避免空完成 failover 的前导事件缓存无限增长。
	openAIResponsesPreOutputBufferMaxBytes = 8 * 1024 * 1024
...
// SetDisabledProxyScheduleModeProvider injects the scheduling-mode provider used
// to filter out accounts whose assigned proxy has been disabled when the
// operator configures "exclude_account" in pool-monitor settings.
// Passing nil restores the default (no filtering beyond DB schedulable flags).
func (s *OpenAIGatewayService) SetDisabledProxyScheduleModeProvider(p DisabledProxyScheduleModeProvider) {
	if s == nil {
		return
	}
...
// InvalidateAgentIdentityWSConnections closes pooled connections after an
// Agent Identity task is re-registered, forcing the next request to handshake
// with a freshly signed assertion.
...
func (s *OpenAIGatewayService) SelectAccountForModelWithExclusions(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*Account, error) {
	return s.SelectAccountForModelWithExclusionsForPlatform(ctx, PlatformOpenAI, groupID, sessionHash, requestedModel, excludedIDs)
}

func (s *OpenAIGatewayService) SelectAccountForModelWithExclusionsForPlatform(ctx context.Context, platform string, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*Account, error) {
	return s.selectAccountForModelWithExclusionsForPlatform(ctx, platform, groupID, sessionHash, requestedModel, excludedIDs, 0)
}
...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusions(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, stickyAccountID int64) (*Account, error) {
	return s.selectAccountForModelWithExclusionsForPlatform(ctx, PlatformOpenAI, groupID, sessionHash, requestedModel, excludedIDs, stickyAccountID)
}
...
func (s *OpenAIGatewayService) tryStickySessionHit(ctx context.Context, groupID *int64, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, stickyAccountID int64) *Account {
	return s.tryStickySessionHitForPlatform(ctx, PlatformOpenAI, groupID, sessionHash, requestedModel, excludedIDs, stickyAccountID)
}
...
func (s *OpenAIGatewayService) tryStickySessionHitForPlatform(ctx context.Context, platform string, groupID *int64, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, stickyAccountID int64) *Account {
	...
	if !account.IsSchedulable() || !isOpenAICompatibleAccountForPlatform(account, platform) {
		return nil
	}
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}) *Account {
	return s.selectBestAccountForPlatform(ctx, PlatformOpenAI, groupID, accounts, requestedModel, excludedIDs)
}
...
func (s *OpenAIGatewayService) selectBestAccountForPlatform(ctx context.Context, platform string, groupID *int64, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}) *Account {
	...
	for _, acc := range candidates {
		verified := s.recheckSelectedOpenAIAccountFromDBForPlatform(ctx, acc, requestedModel, platform)
		if verified != nil {
			return verified
		}
	}
...
func (s *OpenAIGatewayService) SelectAccountWithLoadAwarenessForPlatform(ctx context.Context, platform string, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*AccountSelectionResult, error) {
	...
	if sessionHash != "" {
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			if err == nil {
				...
				if !clearSticky && account.IsSchedulable() && isOpenAICompatibleAccountForPlatform(account, platform) &&
	...
	for i := range accounts {
		...
		if !acc.IsSchedulable() || !isOpenAICompatibleAccountForPlatform(acc, platform) {
			continue
		}
...
func (s *OpenAIGatewayService) listSchedulableAccountsForPlatform(ctx context.Context, groupID *int64, platform string) ([]Account, error) {
	...
	if s.schedulerSnapshot != nil {
		...
		return s.applyDisabledProxyScheduleMode(ctx, accounts), nil
	}
...
// disabled when the operator has chosen "exclude_account" via pool-monitor
// settings. The filtering is a no-op when no provider is wired (unit tests,
// legacy callers) or when the mode is the default "direct_without_proxy".
func (s *OpenAIGatewayService) applyDisabledProxyScheduleMode(ctx context.Context, accounts []Account) []Account {
	if s == nil || s.disabledProxyMode == nil || len(accounts) == 0 {
		return accounts
	}
...
func (s *OpenAIGatewayService) resolveFreshSchedulableOpenAIAccountForPlatform(ctx context.Context, account *Account, requestedModel string, platform string) *Account {
	...
	if !fresh.IsSchedulable() || !isOpenAICompatibleAccountForPlatform(fresh, platform) {
		return nil
	}
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDBForPlatform(ctx context.Context, account *Account, requestedModel string, platform string) *Account {
	...
	if !latest.IsSchedulable() || !isOpenAICompatibleAccountForPlatform(latest, platform) {
		return nil
	}
...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if account.Type == AccountTypeOAuth {
		...
		// Preserve the existing upstream-compatible client metadata behavior:
		// a configured device ID is added even when fingerprint convergence is
		// explicitly disabled. Compact requests use a different unary protocol.
		if !isCompactRequest && applyCodexClientMetadata(reqBody, account) {
			bodyModified = true
			disablePatch()
		}
		// Codex fingerprint convergence: resolve one shared ID set for this
		// request so body metadata and outbound headers remain consistent.
		// Compact requests use a different unary protocol and intentionally skip
		// fingerprint metadata, matching upstream behavior.
		if !isCompactRequest {
...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	for {
		...
		if err != nil {
			...
			c.JSON(http.StatusBadGateway, response.WithErrorCorrelation(c, gin.H{"error": gin.H{"type": "upstream_error", "message": "Upstream request failed"}}))
			return nil, fmt.Errorf("upstream request failed: %s", safeErr)
		}
		if resp.StatusCode < 400 {
			break
		}
		probeBody, _ := io.ReadAll(io.LimitReader(resp.Body, 2<<20))
		_ = resp.Body.Close()
		...
		if shouldFailoverOpenAIPassthroughResponse(account, resp.StatusCode, upstreamMsg, probeBody) {
...
func (s *OpenAIGatewayService) handleFailoverErrorResponsePassthrough(
	...
	body := responseBody
	if len(body) == 0 && resp != nil && resp.Body != nil {
	...
	return &UpstreamFailoverError{
		...
		ResponseBody:           body,
		ResponseHeaders:        resp.Header.Clone(),
		RetryableOnSameAccount: !shouldDisable && account != nil && account.IsPoolMode() && account.IsPoolModeRetryableStatus(resp.StatusCode),
	}
}
...
func openAIResponsesCompletedEventIsEmpty(data []byte, usage *OpenAIUsage) bool {
	...
	if usage != nil && (usage.InputTokens > 0 || usage.OutputTokens > 0 || usage.ImageOutputTokens > 0 ||
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	for scanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if sanitizedData, sanitized := sanitizeOpenAICapacityShedErrorCodeForClient(dataBytes); sanitized {
				dataBytes = sanitizedData
				trimmedData = strings.TrimSpace(string(sanitizedData))
				line = "data: " + string(sanitizedData)
			}
			lineStartsClientOutput = forceFlushFailedEvent || openAIStreamDataStartsClientOutput(trimmedData, eventType)
			if lineStartsClientOutput && trimmedData != "[DONE]" && !isOpenAIResponsesTerminalEvent(eventType) {
			...
			if (eventType == "response.completed" || eventType == "response.done") &&
				...
				return &openaiStreamingResultPassthrough{usage: usage, firstTokenMs: firstTokenMs},
					newOpenAIResponsesEmptyCompletedFailoverError(c, account, upstreamRequestID)
			}
		...
		if !clientDisconnected {
			if !clientOutputStarted && !lineStartsClientOutput {
				...
				if lineBytes > openAIResponsesPreOutputBufferMaxBytes-pendingLineBytes {
					return &openaiStreamingResultPassthrough{usage: usage, firstTokenMs: firstTokenMs},
						s.newOpenAIStreamFailoverError(c, account, true, upstreamRequestID, nil, "OpenAI first-output staging limit exceeded", resp.Header)
				}
			...
			if !clientOutputStarted {
				for _, pendingLine := range pendingLines {
					if _, err := fmt.Fprintln(w, pendingLine); err != nil {
...
func (s *OpenAIGatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string, isStream bool, promptCacheKey string, isCodexCLI bool) (*http.Request, error) {
	...
	// Agent Identity signs a fresh assertion per request; other modes keep
	// their existing Bearer authentication.
	authHeaders, err := s.buildOpenAIAuthenticationHeaders(ctx, account, token)
	if err != nil {
		return nil, fmt.Errorf("build openai authentication headers: %w", err)
	}
	...
	// Apply the precomputed Codex fingerprint IDs after the allowed-header
	// whitelist and before the final OAuth identity normalization.
...
func (s *OpenAIGatewayService) handleStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel string) (*openaiStreamingResult, error) {
	...
	pendingStreamLines := make([]string, 0, 8)
	pendingStreamBytes := 0
	bufferPreOutputLine := func(line string) bool {
		lineBytes := len(line) + 1
		if lineBytes > openAIResponsesPreOutputBufferMaxBytes-pendingStreamBytes {
			streamEarlyErr = s.newOpenAIStreamFailoverError(
				c, account, false, strings.TrimSpace(resp.Header.Get("x-request-id")), nil,
				"OpenAI first-output staging limit exceeded",
			)
			return false
		}
		pendingStreamLines = append(pendingStreamLines, line)
		pendingStreamBytes += lineBytes
		return true
	}
	...
	processSSELine := func(line string, queueDrained bool) {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if eventType == "response.failed" {
				...
				if !clientOutputStarted && openAIStreamFailedEventShouldFailover(dataBytes, failedMessage) {
					sawFailedEvent = true
					streamEarlyErr = s.newOpenAIStreamFailoverError(c, account, false, strings.TrimSpace(resp.Header.Get("x-request-id")), dataBytes, failedMessage, resp.Header)
					return
				}
				forceFlushFailedEvent = true
				sawFailedEvent = true
			...
			if startsClientOutput && !isOpenAIResponsesTerminalEvent(eventType) {
			...
			if account != nil && account.Platform == PlatformOpenAI &&
				...
				streamEarlyErr = newOpenAIResponsesEmptyCompletedFailoverError(c, account, strings.TrimSpace(resp.Header.Get("x-request-id")))
				return
			}
			...
			if !clientDisconnected {
				if !clientOutputStarted && !startsClientOutput {
					bufferPreOutputLine(line)
					return
				}
				if !clientOutputStarted {
					for _, pendingLine := range pendingStreamLines {
						if _, err := bufferedWriter.WriteString(pendingLine + "\n"); err != nil {
							clientDisconnected = true
							break
						}
					}
					pendingStreamLines = pendingStreamLines[:0]
					pendingStreamBytes = 0
				}
				if clientDisconnected {
					return
				...
				if firstTokenMs == nil && openAIStreamDataStartsVisibleOutput(data, eventType) {
...
func extractOpenAIUsageFromJSONBytes(body []byte) (OpenAIUsage, bool) {
	...
	// Preserve OpenAI's native usage precedence while also accepting compatible
	// providers that wrap the response in a top-level data envelope.
	for _, path := range []string{"usage", "response.usage", "data.usage", "data.response.usage"} {
		if usage, ok := extractOpenAIUsageFromGJSON(root, path); ok {
			return usage, true
		}
	}
	return OpenAIUsage{}, false
...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	// 但图片生成请求可能没有常规 token，需要额外检查 ImageOutputTokens 和 ImageCount
	...
	usageLog.ImageCount = result.ImageCount
	usageLog.ImageSize = optionalTrimmedStringPtr(result.ImageSize)
	usageLog.DurationMs = &durationMs
	usageLog.FirstTokenMs = result.FirstTokenMs
	usageLog.CreatedAt = billedAt
	// 设置渠道信息
...
// openAICodexSnapshotStaleForPause mirrors upstream scheduling semantics.
// Missing or invalid timestamps remain conservative (fresh) so incomplete
// historical snapshots do not silently escape an active quota pause.
