...
type openAIWSAcquireRequest struct {
	...
	WSURL   string
	Headers http.Header
	// HeadersFactory is evaluated immediately before each dial so an Agent
	// Identity assertion is never reused by delayed prewarm work.
...
// ClearAccount closes all pooled connections and invalidates delayed prewarm
// work for one account after Agent Identity task recovery.
