...
func (r *apiKeyRepository) Create(ctx context.Context, key *service.APIKey) error {
	builder := r.client.APIKey.Create().
		...
		SetSystemPrompt(key.SystemPrompt).
		SetSystemPromptMode(key.SystemPromptMode).
		SetQuota(key.Quota).
...
func (r *apiKeyRepository) GetByKeyForAuth(ctx context.Context, key string) (*service.APIKey, error) {
	m, err := r.activeQuery().
		...
		Select(
			...
			apikey.FieldGroupID,
			apikey.FieldStatus,
			apikey.FieldSystemPrompt,
			apikey.FieldSystemPromptMode,
		...
		WithUser(func(q *dbent.UserQuery) {
			q.Select(
				...
				user.FieldSalesCommissionMode,
				user.FieldSalesCommissionMinMonthlySales,
			)
		}).
		WithGroup(func(q *dbent.GroupQuery) {
			q.Select(
				...
				group.FieldPlatform,
				group.FieldStatus,
				group.FieldSystemPrompt,
				group.FieldSystemPromptMode,
...
func apiKeyEntityToService(m *dbent.APIKey) *service.APIKey {
	...
	out := &service.APIKey{
		...
		SystemPromptMode: m.SystemPromptMode,
...
func userEntityToService(u *dbent.User) *service.User {
	...
	out := &service.User{
		...
		LegalTermsAccepted:             u.LegalTermsAccepted,
		LegalTermsVersion:              u.LegalTermsVersion,
		LegalTermsAcceptedAt:           u.LegalTermsAcceptedAt,
		UserAccessKeyHash:              u.UserAccessKeyHash,
		UserAccessKeyEncrypted:         u.UserAccessKeyEncrypted,
		UserAccessKeyCreatedAt:         u.UserAccessKeyCreatedAt,
