...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	billingCacheService := service.NewBillingCacheService(billingCache, userRepository, userSubscriptionRepository, subscriptionGrantRepository, apiKeyRepository, configConfig)
	...
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, concurrencyService)
	...
	salesCommissionService := service.ProvideSalesCommissionService(salesCommissionRepository, referralRepository, userRepository)
	...
	userUIPreferencesService := service.NewUserUIPreferencesService(settingRepository)
	userHandler := handler.NewUserHandler(userService, userUIPreferencesService, emailService, emailCache)
	userAccessKeyRepository := repository.NewUserAccessKeyRepository(client)
	userAccessKeyService := service.NewUserAccessKeyService(userAccessKeyRepository, secretEncryptor, configConfig)
	userAccessKeyHandler := handler.NewUserAccessKeyHandler(userService, userAccessKeyService)
	...
	proxyUsageMetricsRepository := repository.NewProxyUsageMetricsRepository(db)
	dashboardService := service.ProvideDashboardService(usageLogRepository, dashboardAggregationRepository, dashboardStatsCache, configConfig, proxyUsageMetricsRepository)
	...
	adminUserHandler := handler.ProvideAdminUserHandler(adminService, concurrencyService, giftBalanceRepository, paymentService)
	...
	schedulerOutboxQueue := repository.ProvideSchedulerOutboxQueue(redisClient)
	schedulerSnapshotService := service.ProvideSchedulerSnapshotService(schedulerCache, schedulerOutboxQueue, accountRepository, groupRepository, configConfig)
	...
	generatedImageObjectStore := repository.NewQiniuGeneratedImageObjectStore(settingService)
	...
	poolAlertConfigRepository := repository.NewPoolAlertConfigRepository(db)
	...
	openAIGatewayService := service.ProvideOpenAIGatewayService(accountRepository, groupRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, openAIImageCache, generatedImageStore, generatedImageObjectStore, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, proxyUsageMetricsRepository, poolMonitorService, giftBalanceRepository)
	...
	generatedImageHandler := admin.NewGeneratedImageHandler(generatedImageStore)
	...
	providerRepository := repository.NewProviderRepository(client, secretEncryptor)
	providerForwarder := service.ProvideProviderForwarder(httpUpstream)
	providerService := service.NewProviderService(providerRepository, accountRepository, providerForwarder, secretEncryptor, apiKeyService)
	providerRouteResolver := service.ProvideProviderRouteResolver(providerRepository)
	providerScheduler := service.ProvideProviderScheduler(concurrencyService)
	providerRouteStateStore := repository.NewProviderRouteStateStore(redisClient)
	providerRouteAttemptRepository := repository.NewProviderRouteAttemptRepository(client)
	providerGatewayService := service.ProvideProviderGatewayService(providerRouteResolver, providerScheduler, providerForwarder, providerRouteStateStore, providerRouteAttemptRepository, providerRouteAttemptRepository)
	providerHandler := admin.NewProviderHandler(providerService, providerGatewayService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, generatedImageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, poolMonitorHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, paymentHandler, referralHandler, salesCommissionHandler, promotionHandler, providerHandler)
	...
	providerGatewayHandler := handler.NewProviderGatewayHandler(providerGatewayService, gatewayService, billingCacheService, apiKeyService, concurrencyService, configConfig)
	handlerSettingHandler := handler.ProvideSettingHandler(settingService, buildInfo)
	totpHandler := handler.NewTotpHandler(totpService)
	groupService := service.NewGroupService(groupRepository, apiKeyAuthCacheInvalidator)
	modelPerformanceMetricsReader := repository.NewModelPerformanceMetricsReader(db)
	modelPricingPageService := service.NewModelPricingPageService(channelService, groupService, billingService, modelPerformanceMetricsReader)
	modelPricingHandler := handler.NewModelPricingHandler(modelPricingPageService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, userAccessKeyHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, providerGatewayHandler, handlerSettingHandler, totpHandler, modelPricingHandler, handlerPaymentHandler, paymentWebhookHandler, handlerReferralHandler, handlerSalesCommissionHandler, channelMonitorUserHandler, idempotencyCoordinator, idempotencyCleanupService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService)
	apiKeyAuthMiddleware := middleware.NewAPIKeyAuthMiddleware(apiKeyService, subscriptionService, configConfig)
	userAccessKeyAuthMiddleware := middleware.NewUserAccessKeyAuthMiddleware(userAccessKeyService)
	...
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, userAccessKeyAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, successfulRequestRecordService, redisClient)
	...
	alertCooldownStore := repository.NewAlertCooldownStore(redisClient)
	alertService := service.NewAlertService(emailService, poolMonitorService, alertCooldownStore)
	proxyConnectivitySnapshotStore := repository.NewProxyConnectivitySnapshotStore(redisClient)
