      ...
      >
        <div class="flex items-center justify-between">
          ...
          <button
            ...
            data-testid="edit-openai-force-chat-completions"
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="w-52">
            <Select
              ...
              data-testid="edit-openai-codex-image-generation-bridge"
            />
          </div>
        </div>
      </div>
...
import type { Account, Proxy, AdminGroup, CheckMixedChannelResponse, OpenAIResponsesMode } from '@/types'
...
function parsePoolModeRetryStatusCodes(input: string): number[] {
  ...
  for (const token of input.split(/[,\s]+/)) {
    ...
    if (!Number.isFinite(n) || !Number.isInteger(n) || n < 100 || n > 599 || seen.has(n)) continue
    seen.add(n)
    out.push(n)
  }
...
function formatPoolModeRetryStatusCodes(value: unknown): string {
  ...
  const out = value
    ...
    .filter(n => Number.isFinite(n) && Number.isInteger(n) && n >= 100 && n <= 599)
...
const resolveCodexImageGenerationBridgeMode = (extra: Record<string, unknown> | undefined): CodexImageGenerationBridgeMode => {
  const value = typeof extra?.codex_image_generation_bridge === 'boolean'
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'apikey')) {
    ...
    if (newAccount.type === 'apikey') {
      openaiForceChatCompletions.value = extra?.openai_responses_mode === 'force_chat_completions'
    }
    codexImageGenerationBridgeMode.value = resolveCodexImageGenerationBridgeMode(extra)
    ...
    if (newAccount.type === 'oauth') {
      ...
      codexFingerprintMode.value = (fpMode === 'off' || fpMode === 'device' || fpMode === 'session' || fpMode === 'full')
        ? fpMode
        : 'session'
    }
  }
  if (newAccount.platform === 'anthropic' && newAccount.type === 'apikey') {
    ...
    anthropicSubUsageAccountingEnabled.value = extra?.anthropic_sub_usage_accounting_enabled === true
  ...
  try {
    ...
    if (props.account.platform === 'anthropic' && props.account.type === 'apikey') {
      ...
      if (anthropicSubUsageAccountingEnabled.value) {
        newExtra.anthropic_sub_usage_accounting_enabled = true
      } else {
        delete newExtra.anthropic_sub_usage_accounting_enabled
      }
      updatePayload.extra = newExtra
    }
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'apikey')) {
      ...
      if (props.account.type === 'apikey') {
        if (openaiForceChatCompletions.value) {
          newExtra.openai_responses_mode = OPENAI_FORCE_CHAT_COMPLETIONS_MODE
        } else {
      ...
      applyCodexImageGenerationBridgeOverride(newExtra)
