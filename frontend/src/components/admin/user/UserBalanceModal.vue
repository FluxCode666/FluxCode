<template>
  <BaseDialog :show="show" :title="operation === 'add' ? t('admin.users.deposit') : t('admin.users.withdraw')" width="narrow" @close="$emit('close')">
    <form v-if="user" id="balance-form" @submit.prevent="handleBalanceSubmit" class="space-y-5">
      ...
      <label v-if="operation === 'add'" class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer select-none">
        <input v-model="form.createOrder" type="checkbox" class="rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-500 dark:bg-dark-700" />
        <span>{{ t('admin.users.createOrderAndAudit') }}</span>
      </label>
...
const submitting = ref(false); const form = reactive({ amount: 0, notes: '', createOrder: false })
watch(() => props.show, (v) => { if(v) { form.amount = 0; form.notes = ''; form.createOrder = false } })
...
const handleBalanceSubmit = async () => {
  ...
  try {
    await adminAPI.users.updateBalance(props.user.id, form.amount, props.operation, form.notes, form.createOrder)
