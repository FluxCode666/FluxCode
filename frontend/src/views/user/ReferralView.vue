      ...
      <template v-else>
          ...
          <div class="p-6">
            ...
            <div v-if="referralInfo?.referral_code" class="mt-4 space-y-3">
              <div class="flex items-center gap-3">
                <code class="flex-1 rounded-lg bg-gray-100 px-4 py-3 text-lg font-mono font-semibold text-primary-600 dark:bg-dark-700 dark:text-primary-400">
              ...
              <button @click="copyLink" class="btn btn-primary w-full">
                <svg class="mr-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.86-2.556a4.5 4.5 0 00-1.242-7.244l-4.5-4.5a4.5 4.5 0 00-6.364 6.364L4.5 8.688" />
                </svg>
                {{ t('referral.copyLink') }}
              </button>
            </div>
        ...
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div class="card p-4 text-center">
            <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ referralInfo?.total_invites ?? 0 }}</div>
            <div class="mt-1 text-xs text-gray-500 dark:text-dark-400">{{ t('referral.totalInvites') }}</div>
          </div>
          <div class="card p-4 text-center">
            <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ referralInfo?.completed_invites ?? 0 }}</div>
            <div class="mt-1 text-xs text-gray-500 dark:text-dark-400">{{ t('referral.completedInvites') }}</div>
          </div>
          <div class="card p-4 text-center">
            <div class="text-2xl font-bold text-primary-600 dark:text-primary-400">${{ (referralInfo?.total_earned ?? 0).toFixed(2) }}</div>
            <div class="mt-1 text-xs text-gray-500 dark:text-dark-400">{{ t('referral.totalEarned') }}</div>
          </div>
          <div class="card p-4 text-center">
            <div class="text-2xl font-bold text-amber-600 dark:text-amber-400">${{ (referralInfo?.gift_balance_remaining ?? 0).toFixed(2) }}</div>
          ...
          <div class="p-6">
            ...
            <div class="mt-4 grid gap-4 sm:grid-cols-3">
              <div class="flex items-start gap-3 rounded-lg bg-gray-50 p-4 dark:bg-dark-700/50">
                <div class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-primary-100 text-sm font-bold text-primary-600 dark:bg-primary-900/30 dark:text-primary-400">1</div>
                <p class="text-sm text-gray-600 dark:text-dark-300">{{ t('referral.step1') }}</p>
              </div>
              <div class="flex items-start gap-3 rounded-lg bg-gray-50 p-4 dark:bg-dark-700/50">
                <div class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-primary-100 text-sm font-bold text-primary-600 dark:bg-primary-900/30 dark:text-primary-400">2</div>
                <p class="text-sm text-gray-600 dark:text-dark-300">{{ t('referral.step2') }}</p>
              </div>
              <div class="flex items-start gap-3 rounded-lg bg-gray-50 p-4 dark:bg-dark-700/50">
                <div class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-primary-100 text-sm font-bold text-primary-600 dark:bg-primary-900/30 dark:text-primary-400">3</div>
              ...
              <table class="w-full text-sm">
                <thead>
                  <tr class="border-b border-gray-200 dark:border-dark-600">
                    <th class="pb-3 pr-4 text-left font-medium text-gray-500 dark:text-dark-400">{{ t('referral.date') }}</th>
                    <th class="pb-3 pr-4 text-left font-medium text-gray-500 dark:text-dark-400">{{ t('referral.status') }}</th>
                    <th class="pb-3 pr-4 text-right font-medium text-gray-500 dark:text-dark-400">{{ t('referral.inviteeReward') }}</th>
                    <th class="pb-3 pr-4 text-right font-medium text-gray-500 dark:text-dark-400">{{ t('referral.inviterReward') }}</th>
                    <th class="pb-3 text-right font-medium text-gray-500 dark:text-dark-400">{{ t('referral.ongoingRewards') }}</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-dark-700">
                  <tr v-for="invite in invites" :key="invite.id">
                    ...
                    <td class="py-3 pr-4 text-right text-gray-700 dark:text-dark-200">${{ invite.invitee_reward_amount.toFixed(2) }}</td>
                    <td class="py-3 pr-4 text-right text-gray-700 dark:text-dark-200">${{ invite.inviter_reward_amount.toFixed(2) }}</td>
                    <td class="py-3 text-right text-gray-700 dark:text-dark-200">
                      {{ invite.ongoing_reward_count > 0 ? `${invite.ongoing_reward_count} ($${invite.ongoing_reward_total.toFixed(2)})` : '-' }}
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div v-else class="overflow-x-auto">
              <table class="w-full text-sm">
                <thead>
                  <tr class="border-b border-gray-200 dark:border-dark-600">
                    <th class="pb-3 pr-4 text-left font-medium text-gray-500 dark:text-dark-400">{{ t('referral.date') }}</th>
                    <th class="pb-3 pr-4 text-left font-medium text-gray-500 dark:text-dark-400">{{ t('referral.source') }}</th>
                    <th class="pb-3 pr-4 text-right font-medium text-gray-500 dark:text-dark-400">{{ t('referral.amount') }}</th>
                    <th class="pb-3 pr-4 text-right font-medium text-gray-500 dark:text-dark-400">{{ t('referral.remaining') }}</th>
                    <th class="pb-3 text-right font-medium text-gray-500 dark:text-dark-400">{{ t('referral.expiresAt') }}</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-dark-700">
                  <tr v-for="record in giftRecords" :key="record.id">
                    ...
                    <td class="py-3 pr-4 text-right text-gray-700 dark:text-dark-200">${{ record.amount.toFixed(2) }}</td>
                    <td class="py-3 pr-4 text-right font-medium" :class="record.remaining > 0 ? 'text-green-600 dark:text-green-400' : 'text-gray-400 dark:text-dark-500'">${{ record.remaining.toFixed(2) }}</td>
                    <td class="py-3 text-right text-gray-700 dark:text-dark-200">
...
async function loadData() {
  loading.value = true
  try {
    const [info, inviteList, records] = await Promise.all([
      getReferralInfo(),
      getMyReferrals(),
      getMyGiftBalanceRecords()
    ])
...
function getSourceLabel(source: string): string {
  switch (source) {
    case 'referral_invitee': return t('referral.sourceReferralInvitee')
    case 'referral_inviter': return t('referral.sourceReferralInviter')
    case 'referral_ongoing': return t('referral.sourceReferralOngoing')
    case 'admin_grant': return t('referral.sourceAdminGrant')
