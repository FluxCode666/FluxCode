...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('admin.usage.traceId'), t('admin.usage.upstreamRequestId'), t('usage.userAgent'), t('admin.usage.ipAddress')
    ]
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.trace_id || '', log.request_id || '', log.user_agent || '', log.ip_address || ''
      ])
      if (rows.length) {
