        ...
        >
          ...
          <template #cell-platform_type="{ row }">
            <div class="flex flex-wrap items-center gap-1">
              <PlatformTypeBadge :platform="row.platform" :type="row.type" :auth-mode="row.credentials?.auth_mode" :plan-type="row.credentials?.plan_type" :privacy-mode="row.extra?.privacy_mode" :subscription-expires-at="row.credentials?.subscription_expires_at" />
              <span
...
const handleExportData = async () => {
  ...
  try {
    ...
    const filename = `fluxcode-account-${timestamp}.json`
