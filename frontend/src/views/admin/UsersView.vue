          ...
          <template #cell-balance="{ value, row }">
            <div class="flex items-start gap-2">
              <div class="group relative">
                ...
                >
                  {{ formatBalanceAmount(getDisplayBalance(row, value)) }}
                </button>
                <div
                  v-if="getGiftBalanceRemaining(row) > 0"
                  class="mt-0.5 text-xs text-gray-500 dark:text-dark-400"
                >
                  {{ formatBalanceAmount(Number(value || 0)) }} + {{ formatBalanceAmount(getGiftBalanceRemaining(row)) }}
                </div>
                <!-- Instant tooltip -->
...
const getGiftBalanceRemaining = (user: AdminUser): number => {
  const value = Number(user.gift_balance_remaining || 0)
  return Number.isFinite(value) ? value : 0
}

const getDisplayBalance = (user: AdminUser, baseBalance?: number): number => {
  const balance = Number(baseBalance ?? user.balance ?? 0)
  return (Number.isFinite(balance) ? balance : 0) + getGiftBalanceRemaining(user)
}

const formatBalanceAmount = (value: number): string => `$${Number(value || 0).toFixed(2)}`
