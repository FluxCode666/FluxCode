    ...
    <BaseDialog :show="showDetailDialog" :title="t('payment.admin.orderDetail')" width="wide" @close="showDetailDialog = false">
      <div v-if="selectedOrder" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div v-if="selectedOrder.promotion_id"><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.promotion') }}</p><p class="text-sm font-medium text-green-600 dark:text-green-400">{{ selectedOrder.promotion_name || '#' + selectedOrder.promotion_id }}</p></div>
          <div v-if="(selectedOrder.discount_amount ?? 0) > 0"><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.promoSaved') }}</p><p class="text-sm font-medium text-orange-600 dark:text-orange-400">-¥{{ (selectedOrder.discount_amount ?? 0).toFixed(2) }}</p></div>
          <div v-if="(selectedOrder.bonus_amount ?? 0) > 0"><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.promoGifted') }}</p><p class="text-sm font-medium text-blue-600 dark:text-blue-400">+${{ (selectedOrder.bonus_amount ?? 0).toFixed(2) }}</p></div>
