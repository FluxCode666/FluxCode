...
import { findModelConflict, validateIntervals } from '@/components/admin/channel/types'
import {
...
// ── Platform config ──
const platformOrder: GroupPlatform[] = ['anthropic', 'openai', 'gemini', 'antigravity', 'embedding']
...
function formToAPI(): { group_ids: number[], model_pricing: ChannelModelPricing[], model_mapping: Record<string, Record<string, string>>, features_config: Record<string, unknown> } {
  return formToChannelAPI(form.platforms, editingChannel.value?.features_config)
}

function apiToForm(channel: Channel): PlatformSection[] {
  return apiToPlatformSections(channel, allGroups.value, platformOrder)
}
...
function distributeRulesToPlatforms(apiRules: AccountStatsPricingRule[]) {
  distributeRulesToPlatformSections(form.platforms, apiRules, allGroups.value)
}
