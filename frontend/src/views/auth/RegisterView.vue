    ...
    <div class="space-y-6">
      ...
      <form v-else @submit.prevent="handleRegister" class="space-y-5">
        ...
        <div v-if="referralEnabled">
          ...
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="userPlus" size="md" :class="referralValidation.valid ? 'text-green-500' : 'text-gray-400 dark:text-dark-500'" />
            </div>
            <input
              id="referral_code"
...
onMounted(async () => {
  try {
    ...
    // Read referral code from URL parameter only if referral is enabled
...
async function handleRegister(): Promise<void> {
  ...
  // Check referral code validation status
  if (formData.referral_code.trim()) {
    if (referralValidating.value) {
      errorMessage.value = t('auth.referralCodeValidating')
      return
    }
    if (referralValidation.invalid) {
      errorMessage.value = t('auth.referralCodeInvalidCannotRegister')
      return
    }
  }
