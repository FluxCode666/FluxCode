#!/usr/bin/env bash
# =============================================================================
# Sub2API - 手动构建并推送后端镜像到 GHCR
# =============================================================================
# 使用 docker buildx 构建多架构后端镜像（不含前端），并推送到 GitHub Container Registry。
#
# 用法:
#   ./deploy/build-and-push-backend.sh 1.0.0            # 构建并推送
#   ./deploy/build-and-push-backend.sh 1.0.0 --no-push  # 仅构建，不推送
#   ./deploy/build-and-push-backend.sh 1.0.0 --local    # 仅构建本机架构，不推送
#
# 前提条件:
...
# ---- 构建 ----
BUILD_ARGS=(
    --file "${REPO_ROOT}/deploy/backend/Dockerfile"
